...
RSpec.describe Waylon::Sense do
  ...
  let(:fake_route) do
    route = instance_double("Route", destination: Waylon::Skills::Default, action: :unknown)
  ...
  it "enqueues work for processing by Skills" do
    expect(Resque).to receive(:enqueue).with(Waylon::Skills::Default, :unknown, resque_details)
    subject.enqueue(fake_route, 123, "some foo")
  end
